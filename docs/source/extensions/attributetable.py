...
TableElement = namedtuple('TableElement', 'fullname label badge subtext')
...
def process_attributetable(app, doctree, fromdocname):
    ...
    for node in node_list:
        ...
        if parent == fullname:
            ...
            for label, subitems in grouped[fullname].items():
                if not subitems:
                    continue
...
def merge_groups(group1: OrderedDict, group2: OrderedDict) -> OrderedDict:
    group = OrderedDict()
    for label, subitems in group1.items():
        group[label] = subitems

    for label, subitems in group2.items():
        if not label in group:
            group[label] = subitems
            continue
...
def get_class_results(lookup, modulename, name, fullname, is_parent):
    ...
    for attr in members:
        ...
        groups[key].append(TableElement(
            ...
            label=label,
            badge=badge,
            subtext=" (inherited)" if not is_parent or override else None
        ))
...
def class_results_to_node(key, elements):
    ...
    for element in elements:
        labels = [nodes.Text(element.label)]
        if element.subtext:
            labels += [nodes.emphasis('', *[nodes.Text(element.subtext)])]
        ref = nodes.reference(
            element.label,
            '',
            internal=True,
